...
// === START: WELCOME KEYBOARD AND UPDATED MESSAGE TEMPLATES ===
// Message templates
...
// === END: WELCOME KEYBOARD AND UPDATED MESSAGE TEMPLATES ===
...
const handleConversationStarted = async (eventData) => {
  try {
    ...
    // === START: MODIFIED WELCOME MESSAGE SENDING IN handleConversationStarted ===
    ...
    // === END: MODIFIED WELCOME MESSAGE SENDING IN handleConversationStarted ===
...
async function handleIncomingMessage(sender, message, messageToken) {
  try {
    ...
    // === START: ADDED LOGIC TO HANDLE BUTTON REPLIES ===
    ...
    if (botResponseText) {
      const responseResult = await viberService.sendViberMessage(sender.id, botResponseText);
    ...
    // === END: ADDED LOGIC TO HANDLE BUTTON REPLIES ===
...
async function handleUserSubscription(sender) {
  try {
    ...
    // === START: MODIFIED WELCOME MESSAGE SENDING IN handleUserSubscription ===
    const welcomeMessageText = createBilingualMessage(
    ...
    const welcomeResult = await viberService.sendViberMessage(sender.id, welcomeMessageObject);
    // === END: MODIFIED WELCOME MESSAGE SENDING IN handleUserSubscription ===
